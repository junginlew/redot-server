...
| HTTP Client | RestTemplate (실제 KIE 호출) — Spring Cloud OpenFeign 의존성은 선언돼 있으나 `@EnableFeignClients` 미설정으로 미사용 |
...
│   ├── TagService.java
│   ├── KieAiClient.java           # RestTemplate 기반 KIE AI 클라이언트 (전략 패턴 선택)
...
2. GenerationService
   ...
   ├── KieAiClient.generateAndSaveImage() → taskId 획득
...
`RestTemplate` 기반 `KieAiClient`로 연동. 전략 패턴(Strategy Pattern)으로 AI 모델별 요청 형식 분리.
